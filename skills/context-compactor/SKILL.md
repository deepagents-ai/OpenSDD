...
6. **Tool result size limit** — the maximum percentage of the context window that a single tool result may occupy. Results exceeding this limit are raw-truncated before being appended to the history. Default: 50% of the context window.
...
The check compares the token count of the tool result against a configurable percentage of the total context window (`toolResultSizeLimit`). If the tool result exceeds this limit, it MUST be raw-truncated to fit within the limit before being appended to the history. A truncation marker MUST be appended to indicate content was lost (see [Raw Truncation](#raw-truncation) for marker requirements).

```
max_result_tokens = context_window_size * tool_result_size_limit
...
| `toolResultSizeLimit` | `number` | A ratio (0.0-1.0) representing the maximum percentage of the context window that a single tool result may occupy. Default: `0.5` (50%). Results exceeding this limit are raw-truncated before being appended to the history. |

## Edge Cases

- **Single tool result exceeds the size limit**: The post-tool-result check MUST raw-truncate the tool result to fit within the configured `toolResultSizeLimit` before it is appended to the history.
