...
- **Turn counter** — the current position within an agent run, rendered as `turn N/M`. Attached ephemerally to every outbound LLM API call (including tool-result calls) without mutating persisted history. Only active when a run-level turn limit is configured.
...
- **Tool result payloads are immutable.** The textual content of a tool result is never modified. Turn-counter attachment on tool-result outbound calls uses sibling content blocks (for schemas that support multi-block user messages) or synthetic messages (for schemas that do not) — never mutation of the tool result payload itself.
- **Persistence is category-dependent.** Stable and dynamic context become part of persisted history (they are part of the human user message as submitted). The turn counter is ephemeral — present in the outbound payload, absent from persisted history.
- **SDK-agnostic design.** The spec defines core concepts (submission hook, pre-request hook, message schema, run boundary) in abstract terms. The implementing agent MUST map each core concept to its concrete equivalent in the specific SDK being used. If the implementing agent cannot identify the concrete equivalent of any core concept, it MUST surface this to the user and ask for guidance rather than guessing or skipping the concept.
...
Dynamic context is NOT re-evaluated on each outbound LLM call within a run. It is evaluated once per submitted user message. Per-call re-evaluation would require ephemeral injection (which the turn counter handles separately) and would invalidate prompt caching on every call.
...
- **Single-string user messages, tool result is a distinct message type (OpenAI-style schemas):**
  - When the latest message in the outbound payload is a human-initiated user message, append the turn-counter string to its content ephemerally. The library MUST restore the content to its persisted form after the outbound call completes (or equivalently, MUST use a non-destructive pre-request hook so the modification is never persisted).
  - When the latest message is a tool result (`role: "tool"` or equivalent), insert a synthetic `role: "user"` message containing only the turn-counter string, immediately after the tool-result message in the outbound payload. This synthetic message MUST NOT be persisted to history.

In both schemas, the attachment MUST be ephemeral — persisted history MUST NOT contain the turn counter in any form.
...
The turn counter, when attached to a user message, comes after stable and dynamic in the outbound payload. The turn counter is ephemeral and never part of the persisted message.
...
All configuration values MUST be modifiable without code changes. The implementing agent MUST choose an appropriate configuration mechanism for the project (e.g., constructor parameters, config file, SDK options).
...
- **Turn counter on turn 1:** The first outbound call of a run typically carries the triggering human user message as the latest message. The turn counter is attached to that outbound payload as specified in [Turn Counter](#turn-counter). The persisted form of the user message in history MUST NOT contain the turn counter.
...
- **SDK only supports destructive pre-request modification:** When the SDK does not distinguish between the outbound payload and the persisted history — i.e., any modification to the outbound payload is saved — the library MUST remove the turn-counter attachment (restore the persisted form) immediately after the outbound call returns, so the next call sees a clean history. If this is not achievable in the SDK, the implementing agent MUST surface this to the user and discuss alternatives (e.g., switching to a manual loop, using the SDK's ephemeral-modification mode if one exists).
...
- Persisted conversation history MUST NOT contain the turn counter in any form at any time.
...
- **Submission-time vs. pre-request timing.** Stable and dynamic injections happen at submission time — they mutate the user message content as it enters the run loop and are persisted as part of normal SDK operation. No special ephemeral-mode mechanism is needed. The turn counter uses a pre-request hook in non-destructive mode. Keep these two flows separate in the implementation.

- **Prefer sibling blocks over content mutation for the turn counter.** In SDKs whose user messages accept multi-block content (Anthropic-style), attaching a sibling text block is strictly cleaner than mutating-and-restoring the content string, because nothing in the original payload needs to be undone after the call.

- **Restoring content after ephemeral mutation.** When the SDK forces single-string content and the library must mutate-then-restore, keep the original unmodified content in a local variable before the outbound call and write it back immediately after. If the SDK does not give the library a post-call hook, consider switching to the synthetic-user-message approach even on turn 1 — two messages in a row with `role: "user"` are accepted by most OpenAI-compatible providers.
